...
_Alternativa descartada_: agregar `soap`/`strong-soap`. Se descarta por ser una dependencia nueva
...
_Alternativa descartada_: lock distribuido en Redis (`SET NX PX`). Se descarta porque la
...
_Riesgo_: el lock se mantiene durante el round-trip HTTP a ARCA (hasta ~10 s por el timeout
