  ...
  if (categories.length === 0) {
    return (
      <div className="p-8 bg-white border border-slate-200 rounded-xl text-center space-y-2">
        ...
        <h3 className="text-sm font-semibold text-slate-900">No hay categorías registradas</h3>
        <p className="text-xs text-slate-500 max-w-sm mx-auto">
          {isAdmin
            ? 'Cree su primera categoría para clasificar los productos del catálogo médico.'
            : 'Actualmente no existen categorías configuradas en el sistema.'}
        </p>
      </div>
    );
  }
          ...
          <tbody className="divide-y divide-slate-100 text-xs text-slate-700">
            {categories.map((cat) => (
              <tr key={cat.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="py-3 px-4 text-slate-500 font-mono">{formatDate(cat.createdAt)}</td>
                {isAdmin && (
