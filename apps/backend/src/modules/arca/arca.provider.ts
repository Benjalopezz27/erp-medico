...
export const arcaServiceProvider: Provider = {
  ...
  useFactory: (configService: ConfigService): IArcaService => {
    ...
    // 2. Development mode -> Mock with 200ms latency
    if (nodeEnv === 'development' && arcaEnv === 'development') {
      return new ArcaMockService({ latencyMs: 200 });
    }

    // 3. Testing mode -> Mock with 0ms latency for fast test runs
    if (nodeEnv === 'test' && arcaEnv === 'development') {
      return new ArcaMockService({ latencyMs: 0 });
    }

    // 4. Prohibited or Invalid configurations -> Fail fast
    if (nodeEnv === 'production') {
      throw new Error(
        `[FATAL] Production ARCA client is not implemented (pending Sprint 8). Set ARCA_ENV=disabled to run in production mode.`,
      );
    }
    ...
    throw new Error(
      `[FATAL] Invalid ARCA environment configuration: NODE_ENV=${nodeEnv}, ARCA_ENV=${arcaEnv}. Allowed values: ARCA_ENV=disabled, or NODE_ENV=development/test with ARCA_ENV=development.`,
    );
  },
